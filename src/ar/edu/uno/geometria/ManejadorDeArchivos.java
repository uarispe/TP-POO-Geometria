...
public class ManejadorDeArchivos {
			...
			try {
				...
				if (tipo.equalsIgnoreCase("SEGMENTO")) {
					...
					if (fig != null && key != null) {
						...
						/*if(figuras.containsKey(key)){ REPASAR
							LinkedList<Figura> listaAux = figuras.get(key);
							listaAux.add(fig);
							figuras.put(key,listaAux);

						}else{
							LinkedList<Figura> listaAux = new LinkedList<Figura>();
							listaAux.add(fig);
							figuras.put(key, listaAux);
						}*/
	...
	private void escribirGrandotes() throws IOException {
		...
		Iterator<Map.Entry<String, LinkedList<Figura>>> itrSF = this.figuras.entrySet().iterator();//REPASAR
		while (itrSF.hasNext()) {
	...
	private void escribirParalelos() throws IOException {
		...
		for (Figura f : listaTriangulos) {
			Triangulo triangulo = (Triangulo) f; //no binarie
